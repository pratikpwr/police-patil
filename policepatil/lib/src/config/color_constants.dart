// ignore_for_file: constant_identifier_names

import 'dart:ui';

import 'package:flutter/material.dart';

const Color PRIMARY_COLOR = Color(0xFFF7770F);
// Colors.blue;
// Color(0xff000eac); // police blue color //Color(0xffdacbb0)
// const Color PRIMARY_COLOR_DARK = Color(0xff3143C8);
// const Color ACCENT_COLOR = Color(0xff4286f5);
const Color POLICE_BLUE = Color(0xff180154);
const Color PRIMARY_COLOR_LIGHT = Color(0xFFf0f6ff);
const Color ORANGE = Color(0xFFF7770F);

const Color BUTTON_TEXT_COLOR = Colors.white;
const Color REGISTER_BUTTON_TEXT_COLOR = Color(0xff424242);
const Color TEXT_COLOR = Color(0xff212121);
const Color TEXT_COLOR_TITLE = Color(0xff212121); //Color(0xff424242);
const Color TEXT_COLOR_SUBTITLE = Color(0xff757575);

const Color SCAFFOLD_COLOR = Color(0xFFE5E5E5);
const Color CONTAINER_BACKGROUND_COLOR = Color(0xFFEFEDED);
